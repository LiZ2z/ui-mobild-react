...
class Slide {
   ...
   * */
  touchStart(e) {
    const changedTouches = e.changedTouches // 新增的触点
  ...
  touchMove(e) {
    ...
    const axis = this.touches[0].axis
    if (this.options.axis && axis && axis !== this.options.axis) return
    ...
    do {
      ...
      this.touches.splice(index, 1, Object.assign({}, item, compute(item, {
...
function compute(cache, current) {
  const currentX = current.currentX
  const currentY = current.currentY
