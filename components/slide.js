...
function compute(cache, current) {
  ...
  // 尽早的判断出滑动的轴线
  const axis = cache.axis || (diffX > diffY ? Axis.X : Axis.Y);
  ...
  return {
    ...
    directionX,
    directionY,
    direction: axis === Axis.X ? directionX : directionY,
    // 当前滑动方向， X轴向上的当前移动方向  Y轴向上当前移动方向
    currentDirectionX: cdX,
    currentDirectionY: cdY,
    currentDirection: axis === Axis.X ? cdX : cdY,
    // 上次触点位置
    currentX,
    currentY,
  };
...
class Slide {
  ...
  touchEnd(e) {
    ...
    const newMap = {};
    for (let i = 0, len = currentTouches.length; i < len; i += 1) {
